...
“系统管理”下的“提示词管理”页面（`/settings/prompts`）支持调整出题和问答评分两套模板；“Token 用量”页面（`/settings/token-usage`）支持按任务和阶段查看模型调用的输入、输出 Token、耗时和失败信息。出题模板支持书名、作者和来源模式变量，管理员修改提示词不能绕过后端的题目结构与来源校验。书籍详情页支持开启一套后台预生成测试，任务期间页面显示生成状态并禁止重复触发；无 PDF 时需要先启用已配置的真实模型，已有但尚未解析完成的 PDF 不会自动降级为模型知识模式。
