...
“提示词管理”页面（`/settings/prompts`）支持调整出题和问答评分两套模板。模板使用受限变量并保存版本，后端会校验必需变量，真实 Provider 会读取当前启用版本；管理员修改提示词不能绕过题目结构和原文来源校验。书籍详情页支持开启一套后台预生成测试，任务期间页面显示生成状态并禁止重复触发；没有完成解析的 PDF 时不会启动预生成。
