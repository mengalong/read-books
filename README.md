...
一个面向个人使用的读书和观影复习网站。用户可以添加书籍、电影、电视剧；上传 PDF 后，系统按原文生成单选题、多选题和问答题，并在提交后给出自动评分、参考答案和可定位到 PDF 页码的原文依据；没有电子版时，也可以只填写标题和主创信息，使用真实模型的内化知识兜底出题。电影和电视剧在保存时会先做模型真实内容检查，未通过时不能依赖模型知识出题。系统会保存生成好的复习试卷；同一套试卷可以反复作答，每次作答单独记录为一个复习任务。
...
考试分享第一版已经可用：用户可以从书籍详情中的已完成试卷生成带有效期的公开考试链接，在“考试管理”查看参与统计和逐份答卷；管理员可以在“系统管理 / 考试管理”查看全平台考试。参与者支持平台账号、微信认证和匿名名称三种身份，重新打开链接时可按对应身份找回历史答卷。公开接口在交卷前隐藏答案与解析，并始终隐藏 PDF 文件名、页码和原文摘录；问答题异步评分失败后可由分享者或管理员重新评分。
