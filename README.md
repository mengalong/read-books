...
系统不会在没有 PDF 的情况下让模型凭记忆生成所谓“原文依据”。模型记忆不能保证版本、章节和引文准确，无法满足本系统的原文可追溯要求；需要可靠出题时请先上传并完成解析 PDF。复习结果页会在圆形分数标识中显示实际得分与满分，并单独展示得分率；得分率低于 60% 时以红色警示。每道题的原文依据会高亮最相关的关键句。
