...
考试分享第一版已经可用：用户可以从书籍详情中的已完成试卷生成公开考试链接，在“考试管理”查看链接状态、日期范围、参与统计和逐份答卷；管理员可以在“系统管理 / 考试管理”查看全平台考试。登录参与者自动使用账号名称，未登录参与者填写答题名称后即可答题。公开接口在交卷前隐藏答案与解析，并始终隐藏 PDF 文件名、页码和原文摘录；问答题异步评分失败后可由分享者或管理员重新评分。
