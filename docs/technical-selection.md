...
真实出题时，后端先选择本次候选 `ContentChunk`，只向模型提供片段 ID、页码和原文内容。模型返回后必须通过题量、题型、选项、正确答案、参考答案、评分要点和来源片段 ID 校验；PDF 文件名、页码与原文摘录由后端根据数据库记录重新构造，不采信模型生成的来源信息。后端同时基于题干、答案和解析从已保存的原文摘录中选择关键句，前端以红色高亮展示，避免模型直接编造高亮内容。客观题继续由后端确定性评分，只有问答题提交时调用模型。
