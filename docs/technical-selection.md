...
出题任务支持实时人工控制：`POST /api/quiz-generation-tasks/{task_id}/cancel` 将 pending/processing 任务标记为 cancelled，后台线程在模型调用返回前后检查该状态，不再写入后续题目或创建试卷；`DELETE /api/quiz-generation-tasks/{task_id}` 删除已终止或失败的任务及其 `model_usage_records`。进行中的任务必须先终止再删除。任务页面可复制整套当前任务或单题的 Prompt、模型回复、草稿和 Token；任何已生成题目在任务完成前都可人工调整，结构/来源校验失败时会从最近一次模型原始回复回填可编辑草稿。
