...
export default function AdminExamManagementPage() {
  ...
  return <div className="page-wrap">
    ...
    {shares.length === 0 ? <EmptyState title="没有匹配的考试" detail="调整搜索词、活动状态或分享用户后再试。" /> : <div className="exam-table-wrap"><table className="exam-table admin-exam-table"><thead><tr><th>考试活动</th><th>分享用户</th><th>状态</th><th>答题情况</th><th>平均得分率</th><th>最近提交</th><th>操作</th></tr></thead><tbody>{shares.map((share) => <tr key={share.id}><td><strong>{share.name}</strong><span>{share.book_title} · {share.quiz_title}</span></td><td><span className="participant-type"><UserRound size={13} />{share.owner_display_name}</span><small>{share.owner_username}</small></td><td><span className={`exam-status exam-status-${share.status}`}>{statusLabels[share.status]}</span>{share.grading_failed_count > 0 && <small className="score-low">{share.grading_failed_count} 份评分失败</small>}</td><td><strong>{share.submitted_count} / {share.started_count}</strong><span>完成率 {share.completion_rate}%</span></td><td>{share.average_score === null ? "—" : `${share.average_score}%`}</td><td>{formatDateTime(share.last_attempt_at)}</td><td><Link aria-label="查看考试详情" className="button button-quiet" href={`/settings/exams/${share.id}`} title="查看详情"><Eye size={15} /></Link></td></tr>)}</tbody></table></div>}
  </div>;
}
