  ...
  return (
    <div className="page-wrap">
      ...
      <section className="result-header">
        <div className={`result-score ${isLowScore ? "low" : ""}`}><div className="score-number"><strong>{percent}</strong><span>分</span></div><div className="score-copy"><div className="eyebrow">Review complete</div><h1>{percent >= 80 ? "掌握得不错，继续保持" : percent >= 60 ? "已经记住一部分" : "找到需要重读的地方了"}</h1><p>{result.title} · 第 {result.attempt_number} 次复习 · 用时 {formatDuration(result.elapsed_seconds)}<br />得分 {result.total_score} / {result.max_score}</p></div></div>
