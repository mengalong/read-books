    ...
    <main className="auth-panel-wrap">
      <section className="auth-panel">
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          <input autoComplete="current-password" id="password" onChange={(event) => setPassword(event.target.value)} placeholder="请输入密码" required type="password" value={password} />
          {error && <div className="auth-form-error">{error}</div>}
          <button className="button button-primary auth-submit" disabled={submitting} type="submit"><LockKeyhole size={16} />{submitting ? "正在登录……" : "登录"}<ArrowRight size={16} /></button>
        </form>
