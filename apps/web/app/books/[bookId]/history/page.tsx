...
export default function HistoryPage() {
  ...
  if (loading) return <div className="page-wrap"><div className="loading-state">正在整理测试历史……</div></div>;
  ...
  return (
    <div className="page-wrap">
      ...
      <header className="page-header"><div><div className="eyebrow">Review history</div><h1 className="page-title">测试历史</h1><p className="page-description">{book.title} · 共完成 {book.stats.quiz_count} 次复习</p></div><Link className="button button-primary" href={`/books/${book.id}/quiz/new`}><Plus size={15} />生成新测试</Link></header>
      {error && <div className="toast-error">{error}</div>}
      {history.length === 0 ? <EmptyState title="还没有测试记录" detail="完成第一套测试后，得分、用时和复习建议会保存在这里。" action={<Link className="button button-primary" href={`/books/${book.id}/quiz/new`}>开始第一次测试</Link>} /> : <div className="history-table-wrap"><table className="history-table"><thead><tr><th>测试</th><th>状态</th><th>得分</th><th>题量</th><th>实际用时</th><th>完成时间</th><th>下次复习</th><th aria-label="操作" /></tr></thead><tbody>{history.map((item) => {
