...
function BankEntryCard({ entry, onSaved }: { entry: QuestionBankEntry; onSaved: (entry: QuestionBankEntry) => Promise<void> }) {
  ...
  return <article className={`question-bank-card ${entry.status}`}>
    ...
    {editing ? <div className="question-bank-edit-form">
      ...
      <details className="question-bank-usages"><summary>查看当前试卷引用（{entry.usages.length}）</summary>{entry.usages.length > 0 ? entry.usages.map((usage) => <div key={usage.id}><span>{usage.quiz_title}{usage.question_position ? ` · 第 ${usage.question_position} 题` : ""}</span><small>{formatDateTime(usage.used_at)}</small></div>) : <p>这道题还没有被其他现存试卷使用。</p>}</details>
    </>}
  </article>;
}
