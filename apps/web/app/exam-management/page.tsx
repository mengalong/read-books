...
export default function ExamManagementPage() {
  ...
  return (
    <div className="page-wrap">
      ...
      {shares.length === 0 ? <EmptyState title="还没有分享考试" detail="从书籍详情页选择一套复习试卷，点击分享按钮即可创建考试链接。" /> : <div className="exam-table-wrap"><table className="exam-table"><thead><tr><th>考试活动</th><th>状态</th><th>答题进度</th><th>成绩</th><th>最近提交</th><th>操作</th></tr></thead><tbody>{shares.map((share) => <tr key={share.id}>
        <td><strong>{share.name}</strong><span>{share.book_title} · {share.quiz_title}</span><small>单选 {share.single_count} · 多选 {share.multiple_count} · 问答 {share.short_count}</small></td>
        <td><span className={`exam-status exam-status-${share.status}`}>{statusLabels[share.status]}</span>{share.grading_failed_count > 0 && <small className="score-low">{share.grading_failed_count} 份评分失败</small>}</td>
        <td><strong>{share.submitted_count} / {share.started_count}</strong><span>完成率 {share.completion_rate}%</span></td>
        <td><strong>{share.average_score === null ? "暂无" : `${share.average_score}%`}</strong><span>最高 {share.highest_score === null ? "暂无" : `${share.highest_score}%`}</span></td>
